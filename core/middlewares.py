...
class LogRequestMiddleware:
    ...
    def __call__(self, request):
        ...
        logger.info(f"Request: {request.method} {request.path} | IP: {ip_addres}")

        response = self.get_response(request)

        logger.info(f"Response: {response.status_code} {response.reason_phrase} for {request.path} | IP: {ip_addres}")
